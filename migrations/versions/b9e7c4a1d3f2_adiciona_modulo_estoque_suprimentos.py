"""adiciona modulo estoque suprimentos

Revision ID: b9e7c4a1d3f2
Revises: f6a3b9d2c8e4
...
revision = "b9e7c4a1d3f2"
down_revision = "f6a3b9d2c8e4"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    existente = bind.execute(
        sa.select(modulos.c.id).where(
            ...
            modulos.c.slug == "estoque",
        )
    ).first()

    dados = {
        "nome": "Estoque",
        "descricao": "Consulta de saldos e historico de movimentacoes de estoque",
        "ativo": True,
        "ordem": 10,
        "atualizado_em": agora,
    }
...
def downgrade():
    ...
    bind.execute(modulos.delete().where(modulos.c.slug == "estoque"))
